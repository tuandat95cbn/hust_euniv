				...
				<div class="form-group">
					...
					<div class='form'>
						<form:select id="staffUniversity" path="staffUniversity" class="form-control" name="staffUniversity">
							<option value="HUST" <c:if test="${staffUniversityCode eq 'HUST'}">selected</c:if>>Giảng viên trong trường</option>
							<option value="DIFF" <c:if test="${staffUniversityCode eq 'DIFF'}">selected</c:if>>Giảng viên ngoài trường</option>
	                    </form:select>
